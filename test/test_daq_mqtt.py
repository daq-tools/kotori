...
from test.settings.mqttkit import settings, cratedb_sensors, influx_sensors, PROCESS_DELAY_MQTT, device_influx_sensors
...
@pytest.mark.mqtt
@pytest.mark.cratedb
def test_mqtt_to_cratedb_json_single(monkeypatch, machinery_cratedb, reset_cratedb):
    """
    Publish single reading in JSON format to MQTT broker
    and proof it is stored in the CrateDB database.
    ...
    yield threads.deferToThread(mqtt_json_sensor, settings.mqtt2_topic_json, data)
